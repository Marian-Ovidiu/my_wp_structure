    ...
    <div class="flex hidden items-center justify-center bg-white sm:block md:block lg:block">
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
            @foreach($progetti as $key => $progetto)
                <div class="out-group mb-4" style="z-index: {{$key+10}}">
                    <div class="flex flex-col items-center justify-center w-full max-w-sm mx-auto">
                        <div class="w-full h-64 bg-gray-300 bg-center bg-cover rounded-lg shadow-md relative" style="background-image: url({{$progetto['immagine']['url']}})">
                        ...
                        <div class="w-full px-4 -mt-10 overflow-hidden rounded-lg shadow-lg bg-gradient-to-t from-custom-green/30 via-custom-green to-custom-green" style="z-index: {{$key+11}}">
                            <h4 class="py-2 font-bold tracking-wide text-center text-gray-800 uppercase dark:text-white">{{$progetto['titolo']}}</h4>
                            <div class="flex items-center justify-center px-3 py-2">
                                <a href="{{$progetto['cta']['url']}}"
